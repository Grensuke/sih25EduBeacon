    ...
    <div className="space-y-6 p-6">
          ...
          {students.map((student) => (
            ...
            >
              ...
              <div className="mt-2 flex items-center gap-2">
                <span className={`w-2 h-2 rounded-full ${(student.riskAnalysis?.overallRiskLevel || student.riskLevel) === 'high' ? 'bg-red-400' : (student.riskAnalysis?.overallRiskLevel || student.riskLevel) === 'medium' ? 'bg-orange-400' : 'bg-green-400'}`}></span>
                <span className="text-xs text-white/70 capitalize">{student.riskAnalysis?.overallRiskLevel || student.riskLevel || 'low'}</span>
              </div>
            </button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-effect rounded-xl shadow-lg p-6">
          ...
          <div className="space-y-2">
            {analytics?.recentActivity?.length === 0 && <p className="text-white/70">No recent activity.</p>}
            {analytics?.recentActivity?.map((act, idx) => (
              <div key={idx} className="flex items-center justify-between p-3 glass-effect rounded-lg">
                <div>
                  <h4 className="font-medium text-white">{act.studentName}</h4>
                  <p className="text-sm text-white/80">{act.text}</p>
                </div>
                <span className="text-xs text-white/60">{new Date(act.date).toLocaleDateString()}</span>
              </div>
            ))}
          </div>
        </div>
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="glass-effect rounded-lg p-4">
                    ...
                    <p className="text-white/80 text-sm">Attendance: {selectedStudent.student.attendanceData?.percentage ?? selectedStudent.student.attendancePct ?? 'N/A'}%</p>
                    <p className="text-white/80 text-sm capitalize">Risk: {selectedStudent.student.riskAnalysis?.overallRiskLevel ?? selectedStudent.student.riskLevel ?? 'low'}</p>
                  </div>
                  <div className="glass-effect rounded-lg p-4">
                    ...
                    <button 
                      ...
                      onClick={() => window.open(`mailto:${selectedStudent.student.parentsContact?.email || ''}?subject=Meeting regarding ${selectedStudent.student.name}'s progress`)}
                    >
                      Schedule Meeting
                    </button>
                  </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="glass-effect rounded-xl shadow-lg p-4">
                    ...
                    {(() => {
                      const level = selectedStudent.student.riskAnalysis?.overallRiskLevel || selectedStudent.student.riskLevel || null;
                      const data = level ? [
